...
use App\Models\SurveiKepuasanMedia;
...
class SurveiKepuasanController extends Controller
{
    ...
    {
        $items = SurveiKepuasan::with('media')->orderBy('position')->latest()->get();
        return view('admin.pelayanan-publik.survei', compact('items'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('logo')) {
                $logoPath = $request->file('logo')->store('survei-kepuasan/logos', 'public');
            }

            $item = SurveiKepuasan::create([
                ...
                'is_published' => true,
                'user_id'      => Auth::id(),
                'position'     => SurveiKepuasan::max('position') + 1,
            ]);
            ...
            if ($request->hasFile('media_files')) {
                ...
                foreach ($request->file('media_files') as $file) {
                    ...
                    SurveiKepuasanMedia::create([
                        'survei_kepuasan_id' => $item->id,
                        'file_path'     => $file->store('survei-kepuasan/media', 'public'),
            ...
            if ($request->hasFile('pdf_files')) {
                ...
                foreach ($request->file('pdf_files') as $file) {
                    SurveiKepuasanMedia::create([
                        'survei_kepuasan_id' => $item->id,
                        'file_path'     => $file->store('survei-kepuasan/pdf', 'public'),
                        'type'          => 'pdf',
                        'original_name' => $file->getClientOriginalName(),
                        'position'      => $pos++,
                    ]);
            ...
            DB::commit();
            return redirect()->route('admin.pelayanan-publik.survei.index')
                ->with('success', __('messages.flash_created', ['item' => __('messages.admin_survei_kepuasan')]));
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    {
        $item = SurveiKepuasan::with('media')->findOrFail($id);
        $items = SurveiKepuasan::with('media')->orderBy('position')->latest()->get();
        return view('admin.pelayanan-publik.survei', compact('item', 'items'));
    }

    public function update(Request $request, $id)
    {
        $item = SurveiKepuasan::findOrFail($id);
        ...
        try {
            ...
            if ($request->has('delete_media')) {
                $toDelete = SurveiKepuasanMedia::whereIn('id', $request->input('delete_media', []))
                    ->where('survei_kepuasan_id', $item->id)->get();
            ...
            DB::commit();
            return redirect()->route('admin.pelayanan-publik.survei.index')
                ->with('success', __('messages.flash_updated', ['item' => __('messages.admin_survei_kepuasan')]));
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function destroy($id)
    {
        ...
        $item->delete();
        return redirect()->route('admin.pelayanan-publik.survei.index')
            ->with('success', __('messages.flash_deleted', ['item' => __('messages.admin_survei_kepuasan')]));
    }
