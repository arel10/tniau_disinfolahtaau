...
@section('title', 'Tambah Halaman ZI')
@section('page-title', 'Tambah Halaman - ' . ucwords(str_replace('_', ' ', $type)))
        ...
        <div class="col-xl-7 col-lg-9">
            <div class="d-flex align-items-center mb-4 gap-3">
                ...
                <div class="flex-grow-1">
                    <h4 class="mb-0 fw-bold text-dark">Tambah Halaman ZI</h4>
                    <small class="text-muted">Tambah halaman Zona Integritas baru.</small>
                </div>
                <a href="{{ route('admin.zi.pages.index', ['type' => $type]) }}" class="ms-auto" style="width:36px;height:36px;border-radius:50%;background:#e9ecef;border:none;color:#555;display:flex;align-items:center;justify-content:center;font-size:1.1rem;text-decoration:none;transition:background 0.2s;flex-shrink:0;" onmouseover="this.style.background='#d0d7e2'" onmouseout="this.style.background='#e9ecef'"><i class="fas fa-times"></i></a>
            </div>
            <div class="card setting-card">
                <div class="card-header">
                    <h6 class="mb-0 fw-bold"><i class="fas fa-file-alt me-2"></i>Data Halaman</h6>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.zi.pages.store', ['type' => $type]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @include('admin.zi-pages._form', ['ziPage' => null])
