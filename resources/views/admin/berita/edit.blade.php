...
@section('title', __('messages.admin_edit_berita'))
@section('page-title', __('messages.admin_edit_berita'))
        ...
        <div class="col-xl-8 col-lg-10">
            <div class="d-flex align-items-center mb-4 gap-3">
                ...
                <div class="flex-grow-1">
                    <h4 class="mb-0 fw-bold text-dark">Edit Berita</h4>
                    <small class="text-muted">Ubah data berita.</small>
                </div>
                    ...
                    <form method="POST" action="{{ route('admin.berita.update', $berita->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane fade show active" id="judul-id">
                                    <input type="text" name="judul" class="form-control @error('judul') is-invalid @enderror" value="{{ old('judul', $berita->judul) }}" required placeholder="Judul dalam Bahasa Indonesia (wajib)">
                                    @error('judul')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="tab-pane fade" id="judul-en">
                                    <input type="text" name="judul_en" class="form-control" value="{{ old('judul_en', $berita->judul_en) }}" placeholder="Title in English (optional)">
                                </div>
                                <div class="tab-pane fade" id="judul-ja">
                                    <input type="text" name="judul_ja" class="form-control" value="{{ old('judul_ja', $berita->judul_ja) }}" placeholder="日本語タイトル (任意)">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="kategori_id" class="form-select @error('kategori_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($kategoris as $kategori)
                                        <option value="{{ $kategori->id }}" {{ old('kategori_id', $berita->kategori_id) == $kategori->id ? 'selected' : '' }}>{{ $kategori->nama_kategori }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                    <option value="draft" {{ old('status', $berita->status) == 'draft' ? 'selected' : '' }}>{{ __('messages.draft') }}</option>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="date" name="tanggal" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', $berita->tanggal ? $berita->tanggal->format('Y-m-d') : '') }}">
                        ...
                        <div class="mb-3">
                            ...
                            @if($berita->gambar_utama)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $berita->gambar_utama) }}" alt="{{ $berita->judul }}" class="img-thumbnail rounded" style="max-height:200px;">
                                </div>
                            @endif
                            <input type="file" name="gambar_utama" class="form-control @error('gambar_utama') is-invalid @enderror" accept="image/*">
                            <small class="text-muted">Biarkan kosong jika tidak ingin mengubah gambar. Format: JPG, PNG, GIF, WEBP (Ukuran bebas)</small>
                        ...
                        <div class="mb-3">
                            ...
                            @if($berita->gambar_tambahan && count($berita->gambar_tambahan))
                                <div class="d-flex flex-wrap gap-2 mb-2">
                                    @foreach($berita->gambar_tambahan as $img)
                                    <div class="position-relative" style="width:120px;">
                                        @if(in_array(pathinfo($img, PATHINFO_EXTENSION), ['mp4','mov','avi','mkv','webm']))
                                            <video src="{{ asset('storage/' . $img) }}" class="img-thumbnail rounded" style="width:120px;height:80px;object-fit:cover;"></video>
                                        @else
                                            <img src="{{ asset('storage/' . $img) }}" class="img-thumbnail rounded" style="width:120px;height:80px;object-fit:cover;">
                                        @endif
                                        <label class="position-absolute top-0 end-0 bg-danger text-white rounded-circle d-flex align-items-center justify-content-center" style="width:22px;height:22px;cursor:pointer;font-size:0.7rem;" title="Hapus file ini">
                                            <input type="checkbox" name="hapus_gambar[]" value="{{ $img }}" class="d-none" onchange="this.closest('.position-relative').style.opacity=this.checked?'0.4':'1'">
                                            <i class="fas fa-times"></i>
                                        </label>
                                    </div>
                                    @endforeach
                                </div>
                                <small class="text-muted d-block mb-1">Klik <strong>X</strong> pada file untuk menghapusnya saat simpan.</small>
                            @endif
                            <input type="file" name="gambar_tambahan[]" class="form-control" accept="image/*,video/*" multiple>
                            <small class="text-muted">Bisa pilih banyak foto/video sekaligus untuk ditambahkan. Format: JPG, PNG, GIF, WEBP, MP4, MOV, AVI, MKV (Ukuran bebas)</small>
                        </div>
                        <div class="mb-3">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane fade show active" id="konten-id">
                                    <textarea name="konten" rows="10" class="form-control @error('konten') is-invalid @enderror" required placeholder="Konten dalam Bahasa Indonesia (wajib)">{{ old('konten', $berita->konten) }}</textarea>
                                    @error('konten')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="tab-pane fade" id="konten-en">
                                    <textarea name="konten_en" rows="10" class="form-control" placeholder="Content in English (optional)">{{ old('konten_en', $berita->konten_en) }}</textarea>
                                </div>
                                <div class="tab-pane fade" id="konten-ja">
                                    <textarea name="konten_ja" rows="10" class="form-control" placeholder="日本語コンテンツ (任意)">{{ old('konten_ja', $berita->konten_ja) }}</textarea>
                                </div>
                            </div>
                        </div>
