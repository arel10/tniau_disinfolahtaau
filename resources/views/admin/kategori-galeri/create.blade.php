        ...
        <div class="col-xl-7 col-lg-9">
            <div class="d-flex align-items-center mb-4 gap-3">
                ...
                <div class="flex-grow-1">
                    <h4 class="mb-0 fw-bold text-dark">Tambah Kategori Galeri</h4>
                    <small class="text-muted">Tambah kategori galeri baru.</small>
                </div>
            ...
            <div class="card setting-card">
                <div class="card-header">
                    <h6 class="mb-0 fw-bold"><i class="fas fa-folder-plus me-2"></i>Data Kategori Galeri</h6>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.kategori-galeri.store') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                <option value="1" {{ old('status', '1') == '1' ? 'selected' : '' }}>{{ __('messages.aktif') }}</option>
                                <option value="0" {{ old('status') == '0' ? 'selected' : '' }}>Nonaktif</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <small class="text-muted">Hanya kategori aktif yang ditampilkan di dropdown.</small>
                        </div>
