...
@section('title', 'Edit Whistle Blowing')
@section('page-title', 'Edit Whistle Blowing')
        ...
        <div class="col-xl-7 col-lg-9">
            <div class="d-flex align-items-center mb-4 gap-3">
                ...
                <div class="flex-grow-1">
                    <h4 class="mb-0 fw-bold text-dark">Edit Whistle Blowing</h4>
                    <small class="text-muted">Ubah data whistle blowing.</small>
                </div>
            ...
            <div class="card setting-card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.whistle-blowing.update', $setting) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label for="judul" class="form-label">Judul <span class="text-danger">*</span></label>
                            <input type="text" name="judul" id="judul" class="form-control @error('judul') is-invalid @enderror" value="{{ old('judul', $setting->judul) }}" required>
                            ...
                            @if($setting->gambar)
                                <div class="mb-2">
                                    @if(str_starts_with($setting->gambar, 'whistle-blowing/'))
                                        <img src="{{ asset('storage/' . $setting->gambar) }}" alt="Gambar" style="max-width:80px;max-height:80px;" class="rounded">
                                    @else
                                        <img src="{{ asset($setting->gambar) }}" alt="Gambar" style="max-width:80px;max-height:80px;" class="rounded">
                                    @endif
                                </div>
                            @endif
                        ...
                        <div class="mb-3">
                            ...
                            <input type="url" name="link_tujuan" id="link_tujuan" class="form-control @error('link_tujuan') is-invalid @enderror" value="{{ old('link_tujuan', $setting->link_tujuan) }}" required>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="is_active" id="is_active" class="form-select @error('is_active') is-invalid @enderror">
                                <option value="1" {{ old('is_active', $setting->is_active) == '1' ? 'selected' : '' }}>{{ __('messages.aktif') }}</option>
