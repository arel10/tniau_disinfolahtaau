...
@section('title', 'Edit Item Penetapan')
@section('page-title', 'Edit Item Penetapan')
        ...
        <div class="col-xl-8 col-lg-10">
            <div class="d-flex align-items-center mb-4 gap-3">
                ...
                <div class="flex-grow-1">
                    <h4 class="mb-0 fw-bold text-dark">Edit Item Penetapan</h4>
                    <small class="text-muted">Ubah data item penetapan.</small>
                </div>
            ...
            <div class="card setting-card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.zi.penetapan.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf @method('PUT')
                        <div class="row">
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select name="kategori_id" class="form-select @error('kategori_id') is-invalid @enderror">
                                    ...
                                    @foreach($kategoris as $kategori)
                                        <option value="{{ $kategori->id }}" {{ old('kategori_id', $item->kategori_id) == $kategori->id ? 'selected' : '' }}>{{ $kategori->nama }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <input type="number" name="persen" class="form-control @error('persen') is-invalid @enderror" value="{{ old('persen', $item->persen) }}" min="0" max="100">
                                @error('persen') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Konten / Deskripsi</label>
                            <textarea name="konten" class="form-control @error('konten') is-invalid @enderror" rows="5">{{ old('konten', $item->konten) }}</textarea>
                        ...
                        <div class="mb-3">
                            ...
                            @if($item->foto)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $item->foto) }}" alt="" style="max-height:150px;border-radius:6px;" class="d-block mb-1">
                                    <label class="form-check-label">
                                        <input type="checkbox" name="hapus_foto" value="1" class="form-check-input"> Hapus foto ini
                                    </label>
                                </div>
                            @endif
