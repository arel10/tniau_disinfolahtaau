...
@section('page-title', __('messages.admin_berita_pelayanan'))
        ...
        <div class="col-xl-10 col-lg-12">
            ...
            <div class="d-flex align-items-center mb-4 gap-3">
                ...
                <div class="flex-grow-1">
                    <h4 class="mb-0 fw-bold text-dark">Berita Pelayanan Publik</h4>
                    <small class="text-muted">Kelola berita pelayanan publik — semua field bersifat opsional, minimal satu harus diisi.</small>
                </div>
            </div>
            ...
            <div class="card setting-card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @if(isset($item))
                    <a href="{{ route('admin.pelayanan-publik.berita.index') }}" class="btn btn-sm btn-outline-light">
                        <i class="fas fa-times me-1"></i>Batal Edit
                    </a>
                    @endif
                </div>
                <div class="card-body p-4">
                    <form action="{{ isset($item) ? route('admin.pelayanan-publik.berita.update', $item->id) : route('admin.pelayanan-publik.berita.store') }}"
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" name="judul" class="form-control"
                                       ...
                                       placeholder="Judul berita (opsional)">
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <input type="file" name="media_files[]" class="form-control" multiple
                                       accept="image/*,video/*" id="mediaFilesInput">
                                ...
                                <div class="mt-2">
                                    ...
                                    <div class="d-flex flex-wrap mt-1">
                                        ...
                                        <div class="media-preview" id="existing-media-{{ $m->id }}">
                                            @if($m->type === 'image')
                            ...
                            <div class="col-12">
                                ...
                                <div class="mt-2">
                                    ...
                                    <div class="d-flex flex-wrap mt-1">
                                        ...
                                        <div class="media-preview" id="existing-pdf-{{ $p->id }}">
                                            <div class="pdf-icon">
            ...
            <div class="card setting-card">
                <div class="card-header">
                    <h6 class="mb-0 fw-bold"><i class="fas fa-list me-2"></i>Daftar Berita Pelayanan Publik ({{ $items->count() }})</h6>
                </div>
                <div class="card-body p-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($row->video_url)
                                        <small class="text-danger"><i class="fab fa-youtube me-1"></i>YouTube</small>
                                        @endif
                                        @if($row->deskripsi)
                                        <small class="text-muted d-block">{{ Str::limit($row->deskripsi, 60) }}</small>
                                        @endif
                                    </td>
                                    <td>
                                        @php
                                            $imgCount = $row->media->where('type','image')->count();
                                            $vidCount = $row->media->where('type','video')->count();
                                            $pdfCount = $row->media->where('type','pdf')->count();
                                        @endphp
                                        @if($imgCount)<span class="badge bg-success me-1"><i class="fas fa-image me-1"></i>{{ $imgCount }}</span>@endif
                                        @if($vidCount)<span class="badge bg-primary me-1"><i class="fas fa-video me-1"></i>{{ $vidCount }}</span>@endif
                                        @if($pdfCount)<span class="badge bg-danger me-1"><i class="fas fa-file-pdf me-1"></i>{{ $pdfCount }}</span>@endif
                                        @if(!$imgCount && !$vidCount && !$pdfCount)<span class="text-muted small">—</span>@endif
                                    </td>
                                    <td>
                                        @if($row->logo_path)
                                    ...
                                    <td>
                                        <form action="{{ route('admin.pelayanan-publik.berita.toggle-publish', $row->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-sm {{ $row->is_published ? 'btn-success' : 'btn-secondary' }}" title="Toggle publikasi">
                                                <i class="fas fa-{{ $row->is_published ? 'eye' : 'eye-slash' }}"></i>
                                            </button>
                                        </form>
                                    </td>
                                    <td class="text-center">
                                        <a href="{{ route('admin.pelayanan-publik.berita.edit', $row->id) }}"
                                           class="btn btn-sm btn-outline-primary me-1" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('admin.pelayanan-publik.berita.destroy', $row->id) }}" method="POST" class="d-inline"
                    ...
                    <div class="py-5 text-center">
                        ...
                        <p class="text-muted mb-0">Belum ada data Berita Pelayanan Publik.</p>
                    </div>
                    @endif
                </div>
            </div>
